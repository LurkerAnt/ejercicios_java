				...
				for (int i = 0; i < entities.size(); i++) {
					...
					if ((e instanceof Mob) && !(e instanceof Player) && ((Mob) e).getHitBox().intersects(hitBox)) {
						...
						if (verticalShot) {
							...
							level.add(new ParticleSpawner((int) x + (width >> 1), (int) y, 10, 5, level,
									Sprite.particle_default));
							level.add(new ParticleSpawner((int) x + width, (int) y, 10, 5, level,
									Sprite.particle_default));
						} else {
							level.add(new ParticleSpawner((int) x, (int) y, 10, 5, level, Sprite.particle_default));
							level.add(new ParticleSpawner((int) x, (int) y + (height >> 1), 10, 5, level,
									Sprite.particle_default));
							level.add(new ParticleSpawner((int) x, (int) y + height, 10, 5, level,
									Sprite.particle_default));
						}
					}
				}
