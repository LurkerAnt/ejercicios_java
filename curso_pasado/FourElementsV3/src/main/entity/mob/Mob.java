...
public abstract class Mob extends Entity {
	...
	public void move(double xa, double ya) {
		...
		// Para que la colision funcione cuando se colisione verticalmente y se pueda
		...
		if (xa > 0)
			dir = Direction.RIGHT; // Right dir
		if (xa < 0)
			dir = Direction.LEFT; // Left dir
		if (ya > 0)
			dir = Direction.DOWN; // Down dir
		if (ya < 0)
			dir = Direction.UP; // Up dir
	...
	private boolean collision(double xa, double ya) {
		...
		for (int corner = 0; corner < 4; corner++) {
			...
			if (this instanceof Player)
				xt = (((int) x + (int) xa) + ((corner % 2) * 12) - 6) >> 4;
			else
				xt = (((int) x + (int) xa) + ((corner % 2) * 12)) >> 4;
	...
	private boolean collisionEntities(double xa, double ya) {
		...
		for (int i = 0; i < level.getEntitiesSize(); i++) {
			...
			if (mob instanceof Mob && !mob.equals(this) && ((Mob) mob).getHitBox().intersects(getNextHitBox(xa, ya))) {
