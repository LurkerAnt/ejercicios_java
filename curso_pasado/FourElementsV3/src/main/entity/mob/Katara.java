...
public class Katara extends Mob {
	...
	private void move() {
		...
		/*
		 * //Follow IA xa = 0; ya = 0; List<Player> players = level.getPlayers(this,
		 * 50); if (players.size() > 0) { Player player = players.get(0); if ((int)x <
		 * (int)player.getX()) xa += speed; if ((int)x > (int)player.getX()) xa -=
		 * speed; if ((int)y > (int)player.getY()) ya -= speed; if ((int)y <
		 * (int)player.getY()) ya += speed; }
		 */
