...
public class Zuko2 extends Mob {
	...
	private int numfireRateHellFire = 900, fireRateHellFire = numfireRateHellFire, rateApocalipsis = 2000,
			apocalipsisTimer = 0;
	...
	private void updateShooting(double distance) {
		...
		if (rateApocalipsis <= 0) {
			...
			if (time % 60 > 58) {
				double dir = dirPlayer();
				for (int i = 0; i < 5; i++) {
					shoot(level.getPlayer().getX() + random.nextInt(128) - 64,
							level.getPlayer().getY() + random.nextInt(128) - 64, 0, dir, 5);
				}
			}
	...
	public void render(Screen screen) {
		...
		if (anim < 420) {
			...
			if (currentHP <= 0) {
				...
				if (deathCounter > 240)
					deathWishSprite = Sprite.zukoLeaving[3];
				else if (deathCounter > 225)
					deathWishSprite = Sprite.zukoLeaving[2];
				else if (deathCounter > 210)
					deathWishSprite = Sprite.zukoLeaving[1];
				else if (deathCounter > 179)
					deathWishSprite = Sprite.zukoLeaving[0];
		...
		if (deathCounter > 179)
			screen.renderSprite((int) (x - 16), (int) (y - 16), deathWishSprite, true);
